...
def group_group_collide(group_1, group_2):
    collisions = 0
    for thing in set(group_1):
        if group_collide(group_2, thing):
            collisions += 1
            group_1.discard(thing)
    return collisions

def draw(canvas):
    global time, started, lives, missile_group, rock_group, score
...
def rock_spawner():
    ...
    if dist(rock_pos, my_ship.get_position()) <= (a_rock.get_radius() + my_ship.get_radius()):
            return
