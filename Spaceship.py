...
class Ship:
    ...
    def update(self):
        ...
        if self.thrust == True:
            ...
            self.vel[0] =  self.vel[0] + acc_koef * orient[0]
            self.vel[1] =  self.vel[1] + acc_koef * orient[1]
            
        self.vel[0] = (1 - friction_koef) * self.vel[0] 
        self.vel[1] = (1 - friction_koef) * self.vel[1] 
        
    def inc_angle(self):
        self.angle_vel += SHIP_ANGLE_VEL_INC
    ...
    def shoot(self):
        ...
        a_missile = Sprite([self.pos[0] + self.radius * angle_to_vector(self.angle)[0],\
                           ...
                           [self.vel[0] + 7 * angle_to_vector(self.angle)[0],\
                            self.vel[1] + 7 * angle_to_vector(self.angle)[1]],\
                           0, 0, missile_image, missile_info, missile_sound)
        
    
# define keyhandlers to control ship
def keydown(key):
...
def draw(canvas):
    ...
    canvas.draw_text('Score: ' + str(score), [610, 50], 40, 'White')
...
def rock_spawner():
    ...
    a_rock = Sprite([random.randrange(WIDTH), random.randrange(HEIGHT)], \
                    [random.random() * random.randrange(-2, 2), random.random() * random.randrange(-2, 2)], \
                    random.random() * 2 * math.pi, 0.1 * random.random() * random.randrange(-1, 1, 2), \
